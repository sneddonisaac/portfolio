...
export default function Simple5050({data}) {
    return (
        <div id={`block_name: ${data?.__typename}, block_id: ${data?.id}`}>
            <Container>
                <div id="content" className="grid grid-rows-3 md:grid-rows-1 md:grid-cols-2">
                    <h1 id="heading" className="text-6xl font-satoshi-medium row-span-1">{data?.heading}</h1>
